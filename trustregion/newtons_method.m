function [optimal_val,optimal_function_minimum] = newtons_method(point,func,grad_func,hess_func)
	...
	while iter < MAX_ITER && fvals(end) > FUNC_TOL
	    ...
	    x = x - inv(d2F(x)) * dF(x);  % gradient descent
    ...
    figure1=figure;
    fvals
	plot(1:(iter-1), fvals, 'LineWidth',2); grid on;
