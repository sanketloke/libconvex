function [optimal_val,optimal_function_minimum]= newtons_method_iterative(point,func,grad_func,hess_func)
    ...
    while (norm(dF(x))>=0.01) & iter<=100
        ...
        if iter>1 & (fvals(iter)-fvals(iter-1))> 0
            break;
        end
        iter=iter+1;
    end
...
function p=approximate_solution(F,dF,d2F,x,trust_radius)
    ...
    if norm(inv(d2F(x))*dF(x)) <=trust_radius 
        ...
        %Iterative method of capturing lambda
        B=d2F(x);
        lambda=iterative_subroutine(B,dF(x),trust_radius);
        if min(eig(B))<0
...
%Iterative method
function lambda=iterative_subroutine(B,gradient,trust_radius)
    %Heuristically add B to make it positive definite and no negative eigen
    %values
    B=B+abs(trace(B)/size(B,1))*eye(size(B));
    lambda=0;
    for i=1:100
       [R,o]=chol(B); %Cholesky Decomposition
       try
        pl=-inv(R'*R)*gradient;
        ql=inv(R')*pl;
       catch % Logs
       ...
       %Update Lambda
       lambda=lambda+(norm(pl)^2/norm(ql)^2)*((norm(pl)-trust_radius)/trust_radius);
    end
end